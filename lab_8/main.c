...
uint8_t main(void)
{
    ...
    adc_init();
    uart_init(BAUD_RATE);
    cyBOT_init_Scan(INIT_SERVO + INIT_IR);

    right_calibration_value = CAL_BOT23_SERVO_R;
    left_calibration_value = CAL_BOT23_SERVO_L;

    // Uncomment and run to find cybot servo callibration values
    /*lcd_init();
    cyBOT_SERVRO_cal_t callibate;
    callibate = cyBOT_SERVO_cal();*/
...
void scanField(uint8_t startAngle, uint8_t endAngle, uint8_t incrementAngle, cyBOT_Scan_t scanner, scanVector vectors[]) {
    ...
    // Iterate through each angle in array
